var counter = 0;
function validateTitleTech(campo) {
...
function validateTitleSalute(campo) {
  return new Promise(function (resolve, reject){
    if(campo==''){
      alert("Inserire titolo");
      resolve(false);
...
$(document).ready(function () {
  ...
  var guidaTech;
  var prodotti =[];
  if($("#Categoria").val()=='Tecnologia'){
    $('#LoadGuida').click(function (event) {
      event.preventDefault();
      titolo = $('#titolo').val();
      introduzione = $('#introduzione').val();
      validateTitleTech(titolo).then(function(result,err) {
        if (result) {
  ...
  else if($("#Categoria").val()=='Salute'){
    $('#LoadGuida').click(function (event) {
      event.preventDefault();
      titolo = $('#titolo').val();
      introduzione = $('#introduzione').val();
      validateTitleSalute(titolo).then(function(result,err) {
        if (result) {
          for (var i = 0; i < counter; i++) {
            prodotti.push({
              posizione:i,
        ...
        $.post(
          ...
          function(response){
            if(response.success){
              alert('GuidaSalute'+titolo+' inserito correttamente');
            }
            else{
              alert(response.extra);
            }
